...
const appEntries = [
  { label: '地址管理', icon: 'i-carbon-location', url: '/pages/staff/address-list' },
  { label: '我的服务', icon: 'i-carbon-user-role', url: '/pages/staff/services' },
  { label: '证件管理', icon: 'i-carbon-id', url: '/pages/staff/certificates' },
  { label: '证件结算', icon: 'i-carbon-document', url: '/pages/staff/settlement' },
]

const otherEntries = [
  { label: '资料认证', icon: 'i-carbon-certificate', url: '/pages/staff/verification' },
  { label: '服务规则', icon: 'i-carbon-document-tasks', url: '/pages/staff/service-rules' },
  { label: '联系客服', icon: 'i-carbon-headset', url: '/pages/staff/contact-service' },
  { label: '设置', icon: 'i-carbon-settings', url: '/pages/staff/settings' },
]

const currentStats = computed(() => statsMap.value?.[period.value])

const statItems = computed(() => [
  { label: '新增订单', value: currentStats.value?.newOrderCount || 0, unit: '单' },
  { label: '待处理', value: currentStats.value?.writeOrderCount || 0, unit: '单' },
  { label: '完成订单', value: currentStats.value?.completedOrderCount || 0, unit: '单' },
  { label: '服务收入', value: formatMoney(currentStats.value?.commissionAmount), unit: '元' },
])
...
async function loadProfile() {
  ...
  staffName.value = profile.staffName || '师傅'
  ...
  <view class="min-h-screen bg-[#F5F7FA] pb-[150rpx]">
    <view class="relative bg-[#FF373D] px-[32rpx] pt-[34rpx] pb-[180rpx]">
      <view class="absolute right-[28rpx] top-[26rpx] flex gap-[24rpx]">
        <text class="i-carbon-chat text-[44rpx] text-white opacity-90" @tap="navigate('/pages/staff/contact-service')" />
        <text class="i-carbon-qr-code text-[44rpx] text-white opacity-90" @tap="navigate('/pages/staff/about')" />
      </view>

      <text class="block text-center text-[44rpx] leading-[54rpx] text-white font-800">个人中心</text>

      <view class="mt-[54rpx] flex items-center">
        <view class="w-[132rpx] h-[132rpx] rounded-full bg-white overflow-hidden flex items-center justify-center" @tap="navigate('/pages/staff/settings')">
          <image :src="avatar" class="w-full h-full" mode="aspectFill" />
        </view>
        <view class="ml-[28rpx] flex-1 min-w-0">
          <view class="flex items-center min-w-0 gap-2" @tap="navigate('/pages/staff/settings')">
          ...
          <view class="mt-[10rpx] flex items-center">
            ...
            <text class="truncate text-[26rpx] text-white opacity-80">师傅ID {{ staffId || '--' }}</text>
          </view>
        </view>
      </view>
    </view>
    ...
    <view class="relative z-1 mx-[24rpx] mt-[-132rpx]">
        ...
        <view class="grid grid-cols-4 mt-[34rpx]">
          <view v-for="item in statItems" :key="item.label" class="flex flex-col items-center">
            <view class="flex items-baseline">
              ...
              <text class="ml-[4rpx] text-[24rpx] text-[#1F2937]">{{ item.unit }}</text>
            </view>
      ...
      <view class="mt-[24rpx] rounded-[28rpx] bg-white p-[28rpx]">
        <view class="grid grid-cols-2 gap-[22rpx]">
          <view class="rounded-[22rpx] bg-[#F7F8FA] p-[28rpx] flex items-center justify-between" @tap="navigate('/pages/staff/settlement')">
            <view>
              <text class="block text-[48rpx] text-[#1F2937] font-700">{{ formatMoney(currentStats?.commissionAmount) }}</text>
              <text class="block mt-[10rpx] text-[28rpx] text-[#8A8F99]">服务金额</text>
            </view>
          ...
          <view class="rounded-[22rpx] bg-[#F7F8FA] p-[28rpx] flex items-center justify-between" @tap="navigate('/pages/staff/settlement')">
            <view>
              ...
              <text class="block mt-[10rpx] text-[28rpx] text-[#8A8F99]">奖金记录</text>
            </view>
      ...
      <view class="mt-[24rpx] rounded-[28rpx] bg-white p-[28rpx]">
        ...
        <view class="grid grid-cols-4 mt-[34rpx]">
          <view v-for="item in appEntries" :key="item.label" class="flex flex-col items-center" @tap="navigate(item.url)">
